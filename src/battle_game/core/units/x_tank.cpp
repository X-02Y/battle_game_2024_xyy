...
x_Tank::x_Tank(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void x_Tank::TankMove(float move_speed, float rotate_angular_speed) {
...
bool x_Tank::IsHit(glm::vec2 position) const {
...
const char *x_Tank::UnitName() const {
  return "x_tank";
}

const char *x_Tank::Author() const {
  return "X-02Y";
}
}  // namespace battle_game::unit
